...
import ro.cs.pub.pubsub.tera.simulation.DroppedAgentsList;
...
public class AgentDropper extends BaseTickerBehaviour<Facilitator>
{
	...
	protected void onTick()
	{
		...
		RandomIterator<AID> iterator = new RandomIterator<AID>(agent.getAllAgents());
		while (dropped.size() < dropCount && iterator.hasNext())
			dropped.add(iterator.next());

		MessageFactory mf = agent.getMessageFactory();
		ACLMessage message = mf.buildMessage(ACLMessage.INFORM,	Names.SIMULATION_CYCLON_RESILIANCE);
